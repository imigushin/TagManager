...
def open_about_popup():
    ...
    about_lbl_regular(
        popup,
        row=4,
        text="This application allow user to combine different tags into 1 line.",
    ...
    about_lbl_regular(
        popup,
        row=5,
        text="Natani Flutesong, nataniflutesong@gmail.com",
        font=helvetica_regular,
    )
    ...
    about_lbl_regular(
        popup,
        row=6,
        text="https://www.buymeacoffee.com/nataniflutesong",
        font=helvetica_regular,
    )
    # Skipper
...
def open_update_log_popup():
    ...
    def update_log_lbl_bold(parent, row, text, font):
        ...
        label.grid(
            row=row, column=0, columnspan=2, padx=(100, 10), pady=(5, 0), sticky="NSEW"
        )
    ...
    lbl_version = Label(
        popup, text="Version: ", font=helvetica_bold, bg="#262d35", fg="#8d9296"
    )
    ...
    update_log_lbl_regular(
        popup,
        row=2,
        text="Added update log section in file menu.\n"
        ...
        "Pack of other smaller fixes and changes.",
        font=helvetica_regular,
    )
...
menu_bar.add_cascade(
    label="Update log", command=open_update_log_popup
)  # Сотри эту строку и верни след-следующую строку !!
# about.add_command(label="Update log", command=open_update_log_popup)

# Header
lbl_Title = Label(
    root,
    text="TAG MANAGER: 1) Choose desired tags, 2) copy generated tags, 3) clear after use. Good luck!",
    font=("Helvetica", 11),
    bg="#262d35",
    fg="#8d9296",
    anchor="w",
...
entry = Text(
    root, width=70, height=3, font=("Helvetica", 15), bg="#38414a", fg="#dde4ea"
)
...
btn_AI = custom_btn(
    root, text="AI tags", command=lambda: input_into_entry("AI fantasy ")
)
btn_Adopt = custom_btn(
    root,
    text="Adopt",
    command=lambda: input_into_entry(
        "Adopt adoptable openadobt open openadoptable digitaldrawing "
    ),
)
btn_Auction = custom_btn(
    root, text="Auction", command=lambda: input_into_entry("Auction auc bid ")
)
...
btn_Fem = custom_btn(
    root, text="Female", command=lambda: input_into_entry("Woman girl female ")
)
...
btn_NB = custom_btn(
    root, text="Non-binary", command=lambda: input_into_entry("Nonbinary NB ")
)
...
btn_ChaShe = custom_btn(
    root,
    text="Character sheet",
    command=lambda: input_into_entry(
        "Character charactersheet reference referencesheet conceptart characterdesign rpg roleplay "
    ),
)
...
btn_Loc = custom_btn(
    ...
    text="Location",
    command=lambda: input_into_entry(
        "Locationpreview location rpg roleplay leveldesign level Locationinterior interior "
    ),
)
btn_Loc.grid(row=7, column=1, padx=5, sticky="NSEW")
btn_WalPor = custom_btn(
    root,
    text="Wallpaper portrait",
    command=lambda: input_into_entry("Wallpaper portrait rpg "),
)
...
btn_DnD = custom_btn(
    ...
    command=lambda: input_into_entry(
        "dnd dungeonanddragons Forgottenrealms pathfinder "
    ),
)
...
btn_WoW = custom_btn(
    ...
    command=lambda: input_into_entry("Warcraft worldofwarcraft "),
)
...
btn_Avi = custom_btn(
    ...
    text="Avian",
    command=lambda: input_into_entry(
        "Avian bird aarakocra kenku aven crowfolk wings anthropomorphic furry "
    ),
)
...
btn_OAB = custom_btn(
    root,
    text="Bear",
    command=lambda: input_into_entry("Bear ursus anthropomorphic furry "),
)
...
btn_Dra = custom_btn(
    ...
    text="Dragonborn",
    command=lambda: input_into_entry(
        "Dragonborn draconoid draconite dragon scalie  anthropomorphic "
    ),
)
...
btn_Sat = custom_btn(
    ...
    text="Draenei/Satyr",
    command=lambda: input_into_entry(
        "Draenei Satyr faun sheep goat anthro anthropomorphic furry "
    ),
)
btn_Sat.grid(row=9, column=2, padx=5, sticky="NSEW")
btn_Dem = custom_btn(
    root, text="Demons", command=lambda: input_into_entry("Demon demonic horns ")
)
btn_Dem.grid(row=10, column=2, padx=5, sticky="NSEW")
btn_Dwa = custom_btn(
    root,
    text="Dwarf",
    command=lambda: input_into_entry("Dwarf beard stocky muscled short "),
)
btn_Dwa.grid(row=11, column=2, padx=5, sticky="NSEW")
btn_Elf = custom_btn(
    root, text="Elf/Drow", command=lambda: input_into_entry("Elf drow ")
)
btn_Elf.grid(row=12, column=2, padx=5, sticky="NSEW")
btn_Fox = custom_btn(
    root,
    text="Fox / Kitsune",
    command=lambda: input_into_entry("Fox kitsune vulpera anthropomorphic furry "),
)
btn_Fox.grid(row=13, column=2, padx=5, sticky="NSEW")
btn_Hal = custom_btn(
    root, text="Halfling", command=lambda: input_into_entry("Halfling short hobbit ")
)
...
btn_OAM = custom_btn(
    ...
    text="Monkey",
    command=lambda: input_into_entry(
        "Monkey gorilla orangutan chimpanzee anthropomorphic furry "
    ),
)
btn_OAM.grid(row=16, column=2, padx=5, sticky="NSEW")
btn_Orc = custom_btn(
    root, text="Orc", command=lambda: input_into_entry("Orc goblinoid greenskin fangs ")
)
btn_Orc.grid(row=17, column=2, padx=5, sticky="NSEW")
btn_OCr = custom_btn(
    root, text="Other creatures", command=lambda: input_into_entry("Creature ")
)
btn_OCr.grid(row=18, column=2, padx=5, sticky="NSEW")
btn_OAR = custom_btn(
    root,
    text="Rabbit",
    command=lambda: input_into_entry("Rabbit hare Lagomorph anthropomorphic furry "),
)
...
btn_Rep = custom_btn(
    ...
    text="Reptile",
    command=lambda: input_into_entry(
        "Reptile lizard scalie scaly lizardman reptilian anthropomorphic "
    ),
)
...
btn_Rod = custom_btn(
    ...
    text="Skaven/Rodent",
    command=lambda: input_into_entry(
        "Rodent skaven mouse rat anthro anthropomorphic furry "
    ),
)
...
btn_Ske = custom_btn(
    root,
    text="Skeleton",
    command=lambda: input_into_entry("Skeleton bones dead boned "),
)
btn_Ske.grid(row=9, column=3, padx=5, sticky="NSEW")
btn_Suc = custom_btn(
    root,
    text="Succubus/Incubus",
    command=lambda: input_into_entry("Succubus incubus demon horns wings tail "),
)
btn_Suc.grid(row=10, column=3, padx=5, sticky="NSEW")
btn_Fel = custom_btn(
    root,
    text="Tabaxi",
    command=lambda: input_into_entry("Tabaxi khajiit cat anthropomorphic furry "),
)
btn_Fel.grid(row=11, column=3, padx=5, sticky="NSEW")
btn_Tau = custom_btn(
    root,
    text="Tauren",
    command=lambda: input_into_entry("Tauren bull cow hooved hooves "),
)
btn_Tau.grid(row=12, column=3, padx=5, sticky="NSEW")
btn_Dev = custom_btn(
    root,
    text="Tiefling/Devils",
    command=lambda: input_into_entry("Tiefling Devil horns tail "),
)
btn_Dev.grid(row=13, column=3, padx=5, sticky="NSEW")
btn_Vam = custom_btn(
    root, text="Vampire", command=lambda: input_into_entry("Vampire fangs ")
)
btn_Vam.grid(row=14, column=3, padx=5, sticky="NSEW")
btn_Wer = custom_btn(
    root,
    text="Werewolf",
    command=lambda: input_into_entry("Werewolf wolf coyote anthropomorphic furry "),
)
...
btn_Arc = custom_btn(
    root, text="Archer", command=lambda: input_into_entry("Archer bow marksman ranged ")
)
btn_Arc.grid(row=6, column=4, padx=5, sticky="NSEW")
btn_Bar = custom_btn(
    root,
    text="Barbarian",
    command=lambda: input_into_entry("Barbarian berserk warrior  muscled "),
)
btn_Bar.grid(row=7, column=4, padx=5, sticky="NSEW")
btn_Bar = custom_btn(
    root, text="Bard", command=lambda: input_into_entry("Bard minstrel ")
)
btn_Bar.grid(row=8, column=4, padx=5, sticky="NSEW")
btn_Def = custom_btn(
    root,
    text="Defender",
    command=lambda: input_into_entry("Defender shield guardian warrior melee "),
)
btn_Def.grid(row=9, column=4, padx=5, sticky="NSEW")
btn_Dru = custom_btn(
    root, text="Druid", command=lambda: input_into_entry("Druid bird haman totem ")
)
btn_Dru.grid(row=10, column=4, padx=5, sticky="NSEW")
btn_Kni = custom_btn(
    root, text="Knight", command=lambda: input_into_entry("Knight armor warrior sword ")
)
btn_Kni.grid(row=11, column=4, padx=5, sticky="NSEW")
btn_Mag = custom_btn(
    root, text="Mage", command=lambda: input_into_entry("Mage elementalist Summoner ")
)
btn_Mag.grid(row=12, column=4, padx=5, sticky="NSEW")
btn_Mar = custom_btn(
    root,
    text="Marksman",
    command=lambda: input_into_entry("Marksman gunner firearm gun rifle "),
)
btn_Mar.grid(row=13, column=4, padx=5, sticky="NSEW")
btn_Pri = custom_btn(
    root, text="Priest", command=lambda: input_into_entry("Priest divine holy saint ")
)
btn_Pri.grid(row=14, column=4, padx=5, sticky="NSEW")
btn_Rog = custom_btn(
    root, text="Rogue", command=lambda: input_into_entry("Rogue dagger knives ")
)
btn_Rog.grid(row=15, column=4, padx=5, sticky="NSEW")
btn_Sam = custom_btn(
    root,
    text="Samurai",
    command=lambda: input_into_entry("Samurai katana warrior melee "),
)
btn_Sam.grid(row=16, column=4, padx=5, sticky="NSEW")
btn_War = custom_btn(
    root, text="Warlock", command=lambda: input_into_entry("Warlock curse ")
)
btn_War.grid(row=17, column=4, padx=5, sticky="NSEW")
btn_Fig = custom_btn(
    root, text="Warrior", command=lambda: input_into_entry("Warrior sword melee ")
)
